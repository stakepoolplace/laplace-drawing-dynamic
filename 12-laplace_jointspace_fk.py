...
# ----------------------------- Encodeur laplacien d'angles (joint-space)
class LaplaceJointEncoder(nn.Module):
    def __init__(self, n_joints=6, K=64, duration=2.0, max_s=0.05):
        ...
        self.a = nn.Parameter(torch.zeros(n_joints, K))
        self.b = nn.Parameter(torch.zeros(n_joints, K))
    ...
    def forward(self, t_grid):
        ...
        decay = torch.exp(-s * t)                     # [T,1,K]
        sinwt = torch.sin(w * t)                      # [T,1,K]
        coswt = torch.cos(w * t)                      # [T,1,K]
        a = self.a.view(1, self.n_joints, self.K)     # [1,J,K]
        b = self.b.view(1, self.n_joints, self.K)     # [1,J,K]
        series = decay * (a * sinwt + b * coswt)      # [T,J,K]
        theta = series.sum(dim=-1) + self.theta0      # [T,J]
        return theta
...
with torch.no_grad():
    encoder.a.mul_(0.0); encoder.b.mul_(0.0)
...
print("🚀 Entraînement (joint-space, FK only)…")
for ep in range(1, args.epochs+1):
    ...
    if ep % 100 == 0 or ep == 1:
        ...
        print(f"Epoch {ep:4d} | Loss {L.item():.5f} | mean |Δp| = {err:.3f} | Z_err = {z_err:.4f} | Z_std = {z_std:.4f}")
...
ax.set_title("Joint-Space Laplace FK — Trajectoire")
