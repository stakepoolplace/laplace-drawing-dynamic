...
class LaplacianOscBank(nn.Module):
    ...
    def __init__(self, bands=3, K_per_band=32, fs=1000.0,
        ...
        # *** NOUVEAUTÉ : Amplitudes complexes c_k = a_k + i*b_k ***
        # Remplace phase qui était sin(wt + phi) → maintenant Re[c * exp(iwt)]
        self.c_real = nn.Parameter(torch.randn(bands, K_per_band) * 0.1)
        self.c_imag = nn.Parameter(torch.randn(bands, K_per_band) * 0.1)
    ...
    def complex_amplitudes(self, band_idx=None):
        """Retourne les amplitudes complexes c_k = a_k + i*b_k pour analyse"""
        if band_idx is None:
            return torch.complex(self.c_real, self.c_imag)
        else:
            return torch.complex(self.c_real[band_idx], self.c_imag[band_idx])
    
    def amplitude_magnitudes(self, band_idx=None):
        """Retourne les magnitudes |c_k| = sqrt(a_k² + b_k²)"""
        if band_idx is None:
            return torch.sqrt(self.c_real**2 + self.c_imag**2)
        else:
            return torch.sqrt(self.c_real[band_idx]**2 + self.c_imag[band_idx]**2)
    
    def amplitude_phases(self, band_idx=None):
        """Retourne les phases arg(c_k) = atan2(b_k, a_k) en radians"""
        if band_idx is None:
            return torch.atan2(self.c_imag, self.c_real)
        else:
            return torch.atan2(self.c_imag[band_idx], self.c_real[band_idx])

    def forward(self, ctx_feat_bandwise, t_fut):
        """
        VERSION COMPLEXE avec c_k = a_k + i*b_k
        
        ctx_feat_bandwise : [B, bands, F]  (features par bande venant du CNN)
        t_fut             : [B, H]         (instants futurs normalisés en secondes)
        Retour            : [B, bands, H]
        
        Formule: y_k(t) = Re[ c_k * exp(-(s_k + i*w_k)*t) ] * g_k(context) * gain_k
        """
        ...
        for b in range(self.bands):
            ...
            # *** CALCUL COMPLEXE ***
            # Créer les amplitudes complexes c_k pour cette bande
            c_real = self.c_real[b].view(1, self.K, 1)  # [1,K,1]
            c_imag = self.c_imag[b].view(1, self.K, 1)  # [1,K,1]
            c_complex = torch.complex(c_real, c_imag)   # [1,K,1] complex64
            
            # Décroissance exponentielle (réelle)
            decay = torch.exp(-s_b * t)  # [B,K,H] via broadcast
            
            # Exponentielle complexe: exp(i*w*t) = cos(w*t) + i*sin(w*t)
            phase = w_b * t  # [B,K,H]
            exp_iwt = torch.complex(torch.cos(phase), torch.sin(phase))  # [B,K,H] complex
            
            # Oscillateur complexe: c * exp(i*w*t)
            complex_osc = c_complex * exp_iwt  # [B,K,H] complex
            
            # Appliquer décroissance et extraire partie réelle
            damped_osc = decay * complex_osc  # [B,K,H] complex
            osc_real = damped_osc.real        # [B,K,H] real
            
            # Gain global
            osc_real = g0 * osc_real          # [B,K,H]

            # Applique gates contextuelles
            gate = g_ctx.view(B, self.K, 1)
            y_b = (gate * osc_real).sum(dim=1, keepdim=True)  # [B,1,H]
            outs.append(y_b)
