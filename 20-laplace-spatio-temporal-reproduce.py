...
"""
Interactive Laplace Spatio-Temporal Demo — micro-movements
----------------------------------------------------------
- 1 : tracé enregistré
- 2 : modèle sur temps bruts (ce qu'il a appris)
- 3 : modèle sur temps EXTRAPOLÉS (durée * (1 + extrap_pct))
- --extrap-pct pour régler le pourcentage d'extrapolation
- --Kt-low et --Kt-high pour régler le nb de neurones (harmoniques) dans les 2 banques
- tracés cumulatifs
- PAS de clamp sur le 3e graphe (on veut voir la vraie dérive)
"""
...
# ============================================================
# 1. Modèle Laplace 2 banques
# ============================================================
class LaplaceMotion2Banks(nn.Module):
    def __init__(
        self,
        duration: float,
        # banque lente
        ...
        max_s_low: float = 2.0,
        # banque rapide (en Hz réels)
        Kt_high: int = 96,
        fmin_high_hz: float = 1.0,
...
# ============================================================
# 6. Replay (3 graphs) dont un d'extrapolation paramétrable
# ============================================================
def replay_raw_times(ts, xs, ys, model, bounds, extrap_pct=0.5, fps=50):
    x_min, x_max, y_min, y_max = bounds
    ...
    # clamp pour 1 et 2 (on garde le cadre réel)
    xy_pred_raw[:, 0] = np.clip(xy_pred_raw[:, 0], x_min, x_max)
    xy_pred_raw[:, 1] = np.clip(xy_pred_raw[:, 1], y_min, y_max)
    # ⚠️ PAS de clamp sur xy_pred_ext → on veut voir la dérive

    # figure
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(14, 5))
    fig.suptitle(f"Recorded vs Laplace (ts bruts) vs Laplace (EXTRAPOLATION {int(extrap_pct*100)}%)")

    # axes 1 et 2 : cadre du dessin
    for ax in (ax1, ax2):
    ...
    # axe 3 : on adapte aux prédictions extrapolées
    # on prend les min/max de l'extrapolation ET du tracé réel, et on ajoute un petit marge
    ...
    ax3.set_title(f"Laplace (extrap. {int(extrap_pct*100)}%)")
    ...
    ANIM = [None]
    max_frames = max(N_rec, N_raw, N_ext)
    ...
    def update(frame):
        ...
        # graph 3 (EXTRAPOLATION, PAS de clamp)
        if frame < N_ext:
...
def main():
    ...
    # banque lente paramétrable
    parser.add_argument("--Kt-low", type=int, default=32, help="nb de neurones (harmoniques) banque lente")
    ...
    # banque rapide paramétrable
    parser.add_argument("--Kt-high", type=int, default=96, help="nb de neurones (harmoniques) banque rapide")
    parser.add_argument("--fmin-high-hz", type=float, default=1.0)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="pourcentage d'extrapolation après la fin (0.5 = +50%, 1.0 = +100%)",
    )
